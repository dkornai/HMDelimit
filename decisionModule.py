...
def get_Name_TauTheta_dict(BPP_outfile):
...
'''
def calculate_GDI(Name_TauTheta_dict, species_1, species_2):    
    ...
    gdi = np.round(1-(np.exp(-2*tau/theta)), decimals = 4)
    
    return gdi

# calculate GDI values for all elements in a list of possible mergeable/splittable pairs
def GDI_Of_Pairs(Name_TauTheta_dict, pairlist):
    output_gdis = []
...
def calculate_tau(Name_TauTheta_dict, species_1, species_2):
...
def age_Of_Pairs(Name_TauTheta_dict, pairlist, mutation_rate:float):
    output_ages = []
    for pair in pairlist:
...
# extract the parameters relevant to the merge decision from the simulation results
def extractModelParam(BPP_outfile, proposed_changes, hm_param):
...
# based on the calculated model paramteres, and the criteria described in the master control file, decide which parameters match
def paramMatch(input_param_dict, hm_param):

    # create empty result dict
    result_dict = {str(pair):{"gdi_1": "?", "gdi_2": "?", "age": "?"} for pair in input_param_dict}
    ...
    for pair in input_param_dict.keys():
        # seperate the variables out for shorter conditional checks
        gdi_1 = input_param_dict[pair]["gdi_1"]
        gdi_2 = input_param_dict[pair]["gdi_2"]
        age   = input_param_dict[pair]["age"]
    
        # if gdi values are available, decide if the gdi value is above or below the threshold 
        if gdi_1 != "?" and gdi_2 != "?":
            if hm_param["mode"] == "merge":
                if gdi_1 <= GDI_thresh:
                    result_dict[str(pair)]["gdi_1"] = True
                else:
                    result_dict[str(pair)]["gdi_1"] = False
                if gdi_2 <= GDI_thresh:
                    result_dict[str(pair)]["gdi_2"] = True
                else:
                    result_dict[str(pair)]["gdi_2"] = False
        ...
        if age != "?":
            if hm_param["mode"] == "merge":
                if age <= age_thresh:
                    result_dict[str(pair)]["age"] = True
                else:
                    result_dict[str(pair)]["age"] = False
            elif hm_param["mode"] == "split":
                if age >= age_thresh:
                    result_dict[str(pair)]["age"] = True
                else:
...
# based on the decision criteria, do the threshold results warrant accepting the proposal    
def criteriaMatch(input_match_dict, hm_param):
...
def decisionUserFeedback(hm_param, proposed_changes, parameters, param_sufficient, decision):
    ...
    # gather the decision parameters and results into list of lists
    ...
    if hm_param["HM_decision"] != "age":
        ...
        for proposal in proposed_changes:
            gdi_1_list.append(str(parameters[str(proposal)]["gdi_1"]))
            gdi_1_acc_list.append(str(param_sufficient[str(proposal)]["gdi_1"]))
            gdi_2_list.append(str(parameters[str(proposal)]["gdi_2"]))
            gdi_2_acc_list.append(str(param_sufficient[str(proposal)]["gdi_2"]))
    ...
    if hm_param['mutationrate'] != "?":
        ...
        for proposal in proposed_changes:
            age_list.append(str(parameters[str(proposal)]["age"]))
            age_acc_list.append(str(param_sufficient[str(proposal)]["age"]))
...
def implement_decision(hm_param, previous_pops:Population_list, decision) -> Population_list:
...
def stop_check(hm_param, decision, new_accepted_pops, halt_pop_number):
